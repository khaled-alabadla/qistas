...
class AuditAction(models.TextChoices):
    ...
    # Finance (Phase 8)
    FEE_AGREEMENT_CREATED = "fee_agreement.created", _("إنشاء اتفاقية أتعاب")
    FEE_AGREEMENT_UPDATED = "fee_agreement.updated", _("تعديل اتفاقية أتعاب")
    FEE_AGREEMENT_STATUS_CHANGED = "fee_agreement.status_changed", _("تغيير حالة اتفاقية أتعاب")
    INVOICE_CREATED = "invoice.created", _("إنشاء فاتورة")
    INVOICE_UPDATED = "invoice.updated", _("تعديل مسودة فاتورة")
    INVOICE_ISSUED = "invoice.issued", _("إصدار فاتورة")
    INVOICE_CANCELLED = "invoice.cancelled", _("إلغاء فاتورة")
    PAYMENT_RECORDED = "payment.recorded", _("تسجيل دفعة")
    PAYMENT_REVERSED = "payment.reversed", _("استرجاع دفعة")
    CREDIT_NOTE_ISSUED = "credit_note.issued", _("إصدار إشعار دائن")
