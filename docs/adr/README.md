...
| [0035](0035-notifications.md) | Notifications: one `Notification` model (a reference, not a copy); five idempotent reminder scans via `generate_notifications` (cron, no worker); date-keyed `dedupe_key`; recipients resolved to who can act then re-checked against the domain capability (invoice = finance.view, paralegal never); recipient-scoped lifecycle, 404 on tamper; in-app only | Yes (Phase 11) |
| [0036](0036-phase-12-hardening.md) | Phase 12 hardening: project-wide security/perf/integrity re-audit (not a feature phase) — fixed an authorization-check-ordering bug in `InvoiceUpdateView` (leaked invoice existence/status to non-finance users) and a client-status mass-assignment/audit-integrity gap; everything else (mass assignment, IDOR, XSS, SQLi, CSRF, document security, finance concurrency, mixed-currency, audit integrity, auth/session, deployment config, dependencies, N+1, DB integrity) re-derived from the code and confirmed sound | Yes (Phase 12) |
